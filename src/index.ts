...
async function sendInvite(url: string, storagePath: string, opts: { headed?: boolean } = {}) {
  ...
  // i18n label patterns
  const CONNECT = /(Connect|Vernetzen|Se connecter|Conectar|Collegati|Conectar-se)/i;
  ...
  try {
    ...
    let clicked = await clickFirstVisible([
      ...
      page.getByRole('main').locator('button:has-text("Connect")').first() // extra fallback
    ]);

    // 2) If not visible, try under “More”
    if (!clicked) {
      const moreBtn = page.getByRole('main').getByRole('button', { name: MORE }).first();
      if (await moreBtn.isVisible().catch(() => false)) {
        await moreBtn.click();
        const overlay = page.locator('[role="menu"], .artdeco-dropdown__content, [role="dialog"]').first();
    ...
    if (!clicked) {
      ...
      if (name) {
        const re1 = new RegExp(`Invite\\s+${escRe(name)}\\s+to\\s+connect`, 'i');
    ...
    if (!clicked) {
      const isPending = await page.getByRole('main').getByRole('button', { name: PENDING }).first()
      ...
      console.log('❌ No Connect found. Accessible header button names:', btnNames);
...
(async () => {
  ...
  if (!url || !storage) {
    console.error('Usage: node linkedin-invite.js "<profile_url>" <storage_state.json> [--headed]');
    process.exit(1);
  }
